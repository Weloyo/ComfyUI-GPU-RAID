...
3. Скрипт сам: ставит ComfyUI (если нет), расширение + GGUF/KJNodes/VHS,
   ...
   вставьте её в панель GPU RAID. Туннель не нужен: RunPod даёт HTTPS-прокси.
4. Модели: скачивайте прямо на под — кнопкой «Скачать на воркера» из панели
   (HF/Civitai URL), либо `wget` в `/workspace/ComfyUI/models/<папка>/`.
