...
5. **Автоподключение** ([docs/LIFECYCLE.md](LIFECYCLE.md)): сначала панель →
   «Подключения и ключи» → GitHub (токен + кнопка «Создать приватный gist»),
   затем пропишите тот же `GIST_ID` в конфиге ноутбука и добавьте GitHub-токен
   в Secrets Colab (🔑 слева, имя **GH_TOKEN**) — воркер появится в панели сам,
   а режимы Эко/«Сразу гасить» смогут освобождать рантайм
   (`runtime.unassign()`) автоматически. Карточка «Google Colab» в подключениях
   держит эти шаги под рукой. API запуска у Colab нет — кнопка «Открыть
   Colab-ноутбук» лишь открывает страницу, Run all нажимаете вы.
