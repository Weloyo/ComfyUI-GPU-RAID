...
PROVIDERS = (
    ...
    },
    {
        ...
        "steps": [
            "Настройте GitHub выше (токен + «Создать приватный gist») — "
            "воркер подключится сам.",
            "Откройте ноутбук; 🔑 Secrets (слева) → добавьте ОДИН раз на "
            "аккаунт: GH_TOKEN — тот же токен, что выше; I_USE_PAID_COLAB — "
            "значение true (подтверждение платного Colab); опционально "
            "MODEL_PRESET (например, minimax_h3) и HF_TOKEN. При первом "
            "запуске разрешите ноутбуку доступ к каждому секрету.",
            "Runtime → Run all — это весь запуск. Ничего вводить не нужно: "
            "gist ноутбук находит сам по токену, конфиг читается из секретов "
            "(копию в Drive сохранять не надо).",
            "Каждый следующий раз: открыть ноутбук → Run all. Гасится воркер "
            "сам (сценарии) или кнопкой ⏻.",
        ],
    },
)
...
def status_view(settings, secrets_view):
    ...
    values = {
        ...
        # памятка секретов Colab: конфиг ноутбук больше не правится — всё
        # читается из Colab Secrets, gist находится сам по GH_TOKEN
        "colab": {"snippet": ("Colab Secrets (🔑, один раз на аккаунт):\n"
                              "  GH_TOKEN         = <токен GitHub, как выше>\n"
                              "  I_USE_PAID_COLAB = true\n"
                              "  MODEL_PRESET     = minimax_h3   (опционально)")
                  if gist_id else ""},
    }
    out = []
    for p in PROVIDERS:
